...
struct ConstantOpInterpreter
    : public InterpreterOpInterface::ExternalModel<ConstantOpInterpreter,
                                                   func::ConstantOp> {
  static EvalResult interpret(Operation *op, Interpreter &interpreter,
                              ArrayRef<EvalValue> operands) {
    ...
    auto func_op =
        interpreter.getModule().lookupSymbol<func::FuncOp>(func_name);
    if (!func_op) {
      return interpreter.createErrorResult(
          "Can't create reference to unknown function " + func_name);
    }
    EvalValue func_ref = interpreter.createEvalValue(op->getResult(0).getType(),
                                                     &func_op, sizeof(func_op));
    llvm::outs() << mli::fmt::dim("Creating reference to function " +
                                  func_name + "\n");
    return interpreter.createBindValueResult(func_ref);
  }
};

struct CallOpInterpreter
    : public InterpreterOpInterface::ExternalModel<CallOpInterpreter,
                                                   func::CallOp> {
  static EvalResult interpret(Operation *op, Interpreter &interpreter,
                              ArrayRef<EvalValue> operands) {
    mli::fmt::printOpName(llvm::outs(), op->getName().getStringRef().str());
...
struct CallIndirectOpInterpreter
    : public InterpreterOpInterface::ExternalModel<CallIndirectOpInterpreter,
                                                   func::CallIndirectOp> {
  static EvalResult interpret(Operation *op, Interpreter &interpreter,
                              ArrayRef<EvalValue> operands) {
    mli::fmt::printOpName(llvm::outs(), op->getName().getStringRef().str());
    auto function_ref = operands[0].getData<func::FuncOp>().front();
    llvm::outs() << mli::fmt::dim("Calling function " +
                                  function_ref.getName().str() +
                                  " from reference\n");
